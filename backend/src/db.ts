...
export class JsonDatabase {
  ...
  async list(collection: CollectionName): Promise<RecordWithId[]> {
    const data = await this.read();
    return clone(data[collection] as RecordWithId[]);
  }

  async create(collection: CollectionName, item: RecordWithId): Promise<RecordWithId> {
    const data = await this.read();
  ...
  async update(collection: CollectionName, id: string, patch: Record<string, unknown>): Promise<RecordWithId> {
    const data = await this.read();
    const items = data[collection] as RecordWithId[];
  ...
  async delete(collection: CollectionName, id: string): Promise<void> {
    ...
    if (nextItems.length === items.length) throw Object.assign(new Error('Record not found'), { status: 404 });
...
export function createTestDatabase() {
  const filePath = path.join(os.tmpdir(), 'upi-cognitive-spend-brake-' + randomUUID() + '.json');
  return new JsonDatabase(filePath, seed);
}
