...
type DemoTokenPayload = { sub: string; tenantId: string; role: Role; iat: number; exp: number };
...
function authSecret() {
  return process.env.DEMO_AUTH_SECRET || 'upi-cognitive-spend-brake-local-demo-secret';
}
...
export function signDemoToken(role: Role, tenantId = 'tenant_demo_bfsi', subject = 'prashant.demo') {
  ...
  const payload = base64UrlEncode(JSON.stringify({ sub: subject, tenantId, role, iat: now, exp: now + 60 * 60 } satisfies DemoTokenPayload));
...
function verifyDemoToken(token: string): DemoTokenPayload | null {
  ...
  if (!isRole(payload.role) || payload.exp < Math.floor(Date.now() / 1000)) return null;
  return payload;
}
...
export function registerAuthRoutes(app: Express) {
  app.post('/api/auth/demo-token', (req, res) => {
    ...
    res.json({
      ...
      principal: { subject: 'prashant.demo', tenantId, role: requestedRole, permissions: permissionsByRole[requestedRole] }
    });
  });
}
